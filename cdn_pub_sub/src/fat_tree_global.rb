	...
	def fill_queries_tor pod_id, tor_id
		1.upto(@pod_size/2) do |host_id|
			...
			host_names.each do |h|
				next if (pod_id.eql?(h[1][:pod_id]) && h[1][:switch_id].eql?(tor_id))

				uplink_port = (@pod_size/2 + 1)
				h_queries = File.open(h[1][:file], "r").read
				old_queries = File.open(switch_queries(tor_sw_name pod_id, tor_id), "r").read

				File.open(switch_queries(tor_sw_name pod_id, tor_id), "a") do |file|
					h_queries.each_line do |l|
						next if(old_queries.include? "#{l.strip}: fwd(#{uplink_port});")
						file.puts "#{l.strip}: fwd(#{uplink_port});"
					end
				end
			end
		end
	...
	def fill_queries_agg pod_id, agg_id
		(@pod_size/2 + 1).upto(@pod_size) do |tor_id|
			...
			1.upto(@pod_size/2) do |host_id|
				...
				# handle upward part
				host_names.each do |h|
					next if pod_id.eql?(h[1][:pod_id])

					uplink_port = 1
					h_queries = File.open(h[1][:file], "r").read
					old_queries = File.open(file_name_, "r").read
