...
let hash_str (s:string) : int =
  let x = ref 0 in
  String.iter s (fun c -> x := !x * 3571 + (Caml.Char.code c));
  !x
...
module Var = struct 
  ...
  let hash (x: t) =
    match x with
    | a, Lt, i -> 1 + (hash_str a) + i
    | a, Gt, i -> 2 + (hash_str a) + i
    | a, Eq, i -> 3 + (hash_str a) + i
...
and node = {uid: uid; var: Var.t; low: bdd_node; high: bdd_node }
and leaf = {leaf_uid: uid; labels: bdd_result list}
...
module NodeH = struct
  ...
  let hash n = 
    let (+) x y = x * 3571 + y in 
    (Var.hash n.var + uid n.low + uid n.high) land Int.max_value
end

module LeafH = struct
  type t = leaf
  let equal = leaf_equal
  let hash l = 
    let (+) x y = x * 3571 + y in 
    (List.fold ~init:0 ~f:(+) l.labels) land Int.max_value
end
...
module LeafWeakHS = Caml.Weak.Make(LeafH)
let leaf_table = LeafWeakHS.create 1337
...
let mk_leaf lbls =
  let l1 = {leaf_uid = !next_uid; labels = lbls} in 
  let l2 = LeafWeakHS.merge leaf_table l1 in 
  if phys_equal l1 l2 
    then incr next_uid;
  L l2
...
type true_or_false_literals =
  | T of Var.t
  | F of Var.t

type conj =
  true_or_false_literals list

let rec conj_to_bdd (formula:conj) lbl =
  match formula with
