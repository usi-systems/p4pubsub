...
from cartracker.p4_pd_rpc.ttypes import *
...
class BaseTest(pd_base_tests.ThriftInterfaceDataPlane):
    def __init__(self):
        pd_base_tests.ThriftInterfaceDataPlane.__init__(self, ["cartracker"])
    ...
    def popCamusEntries(self, app_name, entries):
        ...
        for e in entries:
            ...
            match_spec = globals()['cartracker_%s_match_spec_t' % table_name]

            if 'action_params' in e:
                act_spec = globals()['cartracker_%s_action_spec_t' % act_name]
    ...
    def popCar(self):
        entries_file = os.path.join(this_dir, "car_entries.json")
        with open(entries_file, 'r') as f: entries = json.load(f)
        self.popCamusEntries('car', entries)
    ...
    def popDmac(self):
        ...
        for dmac,egr in self.dmac_tbl.iteritems():
            self.entries['dmac'].append(
                    ...
                    cartracker_dmac_match_spec_t(macAddr_to_string(dmac)),
                    cartracker_set_egress_port_action_spec_t(hex_to_i16(egr))))
    ...
    def popIpv4Lpm(self):
        ...
        for ip,egr in self.ipv4_lpm_tbl.iteritems():
            self.entries['ipv4_lpm'].append(
                    ...
                    cartracker_ipv4_lpm_match_spec_t(ipv4Addr_to_i32(ip), hex_to_i32(0xffffffff)),
                    cartracker_set_egress_port_action_spec_t(hex_to_i16(egr))))
