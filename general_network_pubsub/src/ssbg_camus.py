...
"
    ...
    for i in range(0, f_count):
        ...
        for a_id in attribute_ids:
            ...
            conj.append( ('add_order.x' + repr(a_id), a_descr[0].pick_element(), str(a_descr[1].pick_element())) )

        disj.append(conj)

    if len(disj) < 1:
        return generate_predicate(filters_num_D, attr_num_D, attr_id_D, attributes)
...
"
    ...
    for a_id in attribute_ids:
        a_descr = attributes[a_id]
        conj.append( ('add_order.x' + repr(a_id), str(a_descr[1].pick_element())) )
    return conj

# entry point method
def ssbg(attr_dist_type,
        attr_space_size,
        predicates_size,
        conj_size,
        number_of_filters,
    ...
    #
    attributes_dist = Distribution ({ (str_ops, str_values) : 0, \
                                          (int_ops, int_values) : 100  \
                                          })

    #predicates_dist = Poisson(predicates_size)
    predicates_dist = Distribution({predicates_size: 1})
    ...
    constraint_counts = Distribution({conj_size: 1})
    ...
    for i in range(0, number_of_filters):
        predicates.append(generate_predicate(predicates_dist, constraint_counts, attribute_ids, attributes))
...
Please, see the documentation within the code.""")
    dist_selection = parser.add_mutually_exclusive_group(required=True)
    dist_selection.add_argument('--zipf', help='select attributes using a Zipf probability distribution',
                        action="store_true", default=False)
    dist_selection.add_argument('--uniform', help='select attributes using a uniform probability distribution',
                        action="store_true", default=False)
    parser.add_argument('--attr-space-size', '-a', help='total number of distinct attributes',
                        type=int, default=1)
    parser.add_argument('--disj-size', help='number of conjunctions per filter',
                        type=int, default=1)
    parser.add_argument('--conj-size', help='number of atoms per conjunction',
                        type=int, default=1)
    parser.add_argument('--filters', help='number of generated predicates',
                        type=int, default=1)
    parser.add_argument('--messages', help='number of generated messages',
                        type=int, default=0)
    args = parser.parse_args()

    dist = 'Z' if args.zipf else 'U'

    if False:
        pass
    else:
        predicates, messages = ssbg(
                dist, args.attr_space_size, args.disj_size,
                args.conj_size, args.filters, args.messages)
